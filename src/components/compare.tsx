...
export default function Compare({ onChange }: Props) {
   ...
   useEffect(() => {
      const fetchProviders = async () => {
         let response = await api.get<IProvider[]>(
            "?providers&year=" + selectedOptions.year
         );
   ...
   useEffect(() => {
      const fetchSubjects = async () => {
         let response = await api.get<ISubject[]>(
            "?subjects&year=" +
               selectedOptions.year +
               "&provider=" +
   ...
   useEffect(() => {
      const fetchInfo = async () => {
         let population = await api.get<IPopulation[]>(
            "?populations&year=" +
               selectedOptions.year +
               "&provider=" +
               selectedOptions.provider +
         ...
         let level = await api.get<ILevel[]>(
            "?levels&year=" +
               selectedOptions.year +
               "&provider=" +
               selectedOptions.provider +
